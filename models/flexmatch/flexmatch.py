  ...
  def train(self, args, logger=None):
    ...
    for (_, x_lb, y_lb), (x_ulb_idx, x_ulb_w, x_ulb_s) in zip(self.loader_dict['train_lb'],
      ...
      with amp_cm():
        ...
        targets_total = torch.cat((y_lb, pseudo_lb), dim=0)
        inputs_total = torch.cat((x_lb, x_ulb_w, ))

        model_with_temperature = ModelWithTemperature(self.model)
        pseudolabeled_dataset = BasicDataset(alg = None,
                                             data = inputs_total.cpu().detach(),
                                             targets = targets_total.cpu().detach())
        ...
        self..T = model_with_temperature.temperature.item()
